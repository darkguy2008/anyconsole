...
RUN echo "Acquire::http::Proxy \"$APT_PROXY\"; Acquire::https::Proxy \"DIRECT\";" > /etc/apt/apt.conf.d/01proxy \
 ...
 && apt-get update \
 && apt-get install -y --no-install-recommends busybox gcc libc6-dev libsdl2-dev libevdev-dev pkgconf libdrm-dev libpng-dev libpixman-1-dev libcairo2-dev libjson-c-dev libwayland-dev wayland-protocols librsvg2-bin fonts-inter fontconfig-config btop ncurses-base rsync grub-common fdisk e2fsprogs dosfstools pax-utils dropbear-bin dhcpcd-base libwlroots-0.18-dev libxkbcommon-dev libegl-dev libgles-dev libvulkan-dev mesa-vulkan-drivers wayland-utils udev kmod pipewire pipewire-bin pipewire-pulse wireplumber cifs-utils jq docker-ce docker-ce-cli containerd.io docker-buildx-plugin libgl1-mesa-dri mesa-libgallium libegl-mesa0 \
